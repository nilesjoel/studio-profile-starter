...
import { getToken } from "next-auth/jwt"
 ...
 export async function getSocialPostsData(req: NextApiRequest, res: NextApiResponse) {

    const { jwt } = await getToken({ req })
    const token = await getServerSession(req, res, authOptions)
    console.log({ token })
    try {
        const response = await fetch(`${process.env.NEXTAUTH_API}/studio-social/posts/profile`, {
            method: 'POST',
            headers: {
